...
const LiveDashboard = () => {
  ...
  const barOptions: any = {
    ...
    scales: {
      x: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#93B1B5' } },
      y: { grid: { display: false }, ticks: { color: '#B8E3E9', font: { family: 'Rajdhani', size: 14 } } },
    },
  };
  ...
  const donutOptions: any = {
    ...
    cutout: '65%',
    plugins: {
      legend: { position: 'bottom' as const, labels: { color: '#93B1B5', usePointStyle: true } },
      tooltip: {
        callbacks: {
          label: (ctx: any) => {
            const p = parties[ctx.dataIndex];
    ...
    <AnalyticsLayout>
      <div className="pb-24">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-4 md:px-8 pt-6">
          <div className="bg-[#143e44] border border-[#4F7C82] border-l-4 !border-l-[#93B1B5] rounded-xl p-5">
            <div className="flex items-center gap-2 text-xs font-heading uppercase tracking-wide text-[#93B1B5]">
              <Users size={16} /> REGISTERED VOTERS
            </div>
            <div className="font-heading text-4xl font-bold text-[#B8E3E9] mt-2">{liveTurnout.totalRegistered.toLocaleString()}</div>
            <div className="text-xs text-[#93B1B5] mt-1">Karnataka State Constituency</div>
          </div>

          <div className="bg-[#103a40] border border-[#4F7C82] rounded-xl p-5">
            <div className="flex items-center gap-2 text-xs font-heading uppercase tracking-wide text-[#93B1B5]">
          ...
          <div className="bg-[#143e44] border border-[#4F7C82] border-l-4 !border-l-[#e65100] rounded-xl p-5">
            <div className="flex items-center gap-2 text-xs font-heading uppercase tracking-wide text-[#93B1B5]">
          ...
          <div className="bg-[#143e44] border border-[#4F7C82] border-l-4 !border-l-[#546e7a] rounded-xl p-5">
            <div className="flex items-center gap-2 text-xs font-heading uppercase tracking-wide text-[#93B1B5]">
              <Clock size={16} className="text-[#546e7a]" /> POLLING CLOSES
            </div>
            <div className={`font-heading text-2xl font-bold mt-2 ${countdown.closed ? 'text-red-400' : 'text-[#B8E3E9]'}`}>
              {countdown.text}
            </div>
            <div className="text-xs text-[#93B1B5] mt-1">Polling closes at 6:00 PM</div>
          </div>
        </div>

        {/* Party Vote Cards */}
        <div className="px-4 md:px-8 mt-6">
          <div className="font-heading font-bold text-xs uppercase tracking-widest text-[#93B1B5] mb-3">LIVE PARTY STANDINGS</div>
            ...
            {sortedParties.map((p, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm text-[#93B1B5] mt-1">{pct}%</div>
                  ...
                  <div className="mt-2 text-xs">
                    {diff > 0 ? <span className="text-green-400">▲ +{diff}</span> : <span className="text-[#93B1B5]">— No change</span>}
                  </div>
                  {idx === 0 && (
        ...
        <div className="px-4 md:px-8 mt-6 grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-3 bg-[#143e44] border border-[#4F7C82] rounded-xl p-6">
            <div className="font-heading font-bold text-[#B8E3E9]">Party-wise Vote Distribution</div>
            <div className="text-xs text-[#93B1B5] mb-3">Live data • Updates every 30 seconds</div>
            <div style={{ height: 280 }}><Bar data={barData} options={barOptions} /></div>
          </div>
          <div className="lg:col-span-2 bg-[#143e44] border border-[#4F7C82] rounded-xl p-6">
            <div className="font-heading font-bold text-[#B8E3E9] mb-3">Vote Share %</div>
        ...
        <div className="px-4 md:px-8 mt-6 mb-8">
          <div className="bg-[#143e44] border border-[#4F7C82] rounded-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[#4F7C82] flex justify-between items-center">
              <div className="font-heading font-bold text-[#B8E3E9] text-sm uppercase tracking-wide">CONSTITUENCY-WISE RESULTS</div>
              <div className="text-xs text-[#93B1B5]">{constituencies.length} constituencies</div>
            </div>
                  ...
                  {constituencies.map((c, i) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`${i % 2 === 0 ? 'bg-[#143e44]' : 'bg-[#103a40]'} hover:bg-[#1c4f55] cursor-pointer transition-colors`}
                      >
                        <td className="px-6 py-3 text-[#B8E3E9] font-semibold">{c.name}</td>
                        <td className="px-6 py-3 text-[#B8E3E9]">
                        ...
                        <td className="px-6 py-3">
                          <span
                            ...
                            style={{ color: partyMeta?.color, borderColor: (partyMeta?.color ?? '#93B1B5') + '4D', backgroundColor: (partyMeta?.color ?? '#93B1B5') + '26' }}
                          >
      ...
      {/* Bottom bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#103a40] border-t border-[#4F7C82] px-4 md:px-8 py-3 flex flex-wrap justify-between items-center gap-3 no-print z-40">
        <div className="text-xs text-[#93B1B5]">Last updated: {new Date().toLocaleTimeString()}</div>
        <button onClick={closePolling} className="text-xs text-[#93B1B5] hover:text-white">Simulate Polling Close (Demo)</button>
        <div className="flex gap-3">
          <button onClick={() => navigate('/analytics/booths')} className="border border-[#4F7C82] text-[#93B1B5] hover:border-[#4F7C82] hover:text-white rounded-lg px-4 py-1.5 text-xs">
            View Booth Turnout →
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-1.5 text-xs border ${
              liveTurnout.pollingOpen
                ? 'opacity-50 cursor-not-allowed text-[#93B1B5] border-[#4F7C82]'
