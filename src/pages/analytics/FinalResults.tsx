...
const FinalResults = () => {
  ...
  const partyColor = (abbr: string) => parties.find(p => p.abbreviation === abbr)?.color ?? '#93B1B5';
    ...
    <AnalyticsLayout>
      ...
      <div className="pb-12">
        ...
        >
          ...
          <div className="font-heading text-2xl text-[#B8E3E9]/80 mt-2">WINS KARNATAKA CONSTITUENCY</div>
          <div
        ...
        <div className="px-4 md:px-8">
          <Link to="/analytics/dashboard" className="inline-block mt-6 text-[#93B1B5] hover:text-white text-sm no-print">← Back to Dashboard</Link>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {[
              ...
              <div key={s.l} className="bg-[#143e44] border border-[#4F7C82] rounded-xl p-5">
                <div className="text-xs uppercase tracking-wide text-[#93B1B5] font-heading">{s.l}</div>
                <div className="font-heading text-3xl font-bold text-[#B8E3E9] mt-1">{s.v}</div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-[#143e44] border border-[#4F7C82] rounded-xl overflow-hidden mt-6">
            <div className="px-6 py-4 border-b border-[#4F7C82] font-heading font-bold text-[#B8E3E9]">Final Vote Count — All Parties</div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {sorted.map((p, i) => {
                    ...
                    const rankColor = i === 0 ? '#ffd700' : i === 1 ? '#c0c0c0' : i === 2 ? '#cd7f32' : '#93B1B5';
                    const isWinner = i === 0;
                    const margin = isWinner ? '—' : `-${winner.votes - p.votes} votes`;
                    return (
                      <tr key={p.id}
                        className={`${isWinner ? '' : i % 2 === 0 ? 'bg-[#143e44]' : 'bg-[#103a40]'} hover:bg-[#1c4f55]`}
                      ...
                      >
                        ...
                        <td className="px-6 py-3 text-[#B8E3E9] font-semibold">{p.name}</td>
                        <td className="px-6 py-3 text-2xl">{p.emoji}</td>
                        <td className="px-6 py-3 text-[#B8E3E9]">{p.votes.toLocaleString()}</td>
                        <td className="px-6 py-3">
                          <div className="text-[#B8E3E9] text-sm">{pct}%</div>
                        ...
                        <td className="px-6 py-3">
                          {isWinner
                            ...
                            : <span className="text-[#93B1B5] text-xs">—</span>}
                        </td>
          ...
          {/* Final bar chart */}
          <div className="bg-[#143e44] border border-[#4F7C82] rounded-xl p-6 mt-6">
            <div className="font-heading font-bold text-[#B8E3E9] mb-3">Final Results — Vote Distribution</div>
            <div style={{ height: 280 }}><Bar data={barData} options={barOptions} /></div>
          </div>

          {/* Constituency winners */}
          <div className="bg-[#143e44] border border-[#4F7C82] rounded-xl overflow-hidden mt-6">
            <div className="px-6 py-4 border-b border-[#4F7C82] font-heading font-bold text-[#B8E3E9]">Constituency-wise Winners</div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#0B2E33]">
                  <tr>
                ...
                <tbody>
                  {constituencies.map((c, i) => {
                    ...
                    return (
                      <tr key={c.id} className={i % 2 === 0 ? 'bg-[#143e44]' : 'bg-[#103a40]'}>
                        ...
                        <td className="px-6 py-3 text-[#B8E3E9]">{w.votes}</td>
                        <td className="px-6 py-3 text-[#93B1B5]">{r?.party ?? '—'}</td>
                        <td className="px-6 py-3 text-xs text-green-400">+{w.votes - (r?.votes ?? 0)}</td>
                        <td className="px-6 py-3 text-[#B8E3E9]">{c.turnout}%</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="flex gap-4 mt-6 no-print">
            <button onClick={() => window.print()} className="border border-[#4F7C82] text-[#B8E3E9] hover:bg-[#1c4f55] rounded-xl px-6 py-3">🖨️ Export as PDF</button>
            <button onClick={generateCSV} className="border border-[#4F7C82] text-[#4F7C82] hover:bg-[#103a40] rounded-xl px-6 py-3">📊 Download CSV</button>
          </div>
        </div>
      </div>
    </AnalyticsLayout>
