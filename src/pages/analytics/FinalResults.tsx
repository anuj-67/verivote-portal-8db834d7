...
const FinalResults = () => {
  ...
  const partyColor = (abbr: string) => parties.find(p => p.abbreviation === abbr)?.color ?? '#7986cb';
    ...
    <AnalyticsLayout>
      ...
      <div className="pb-12">
        ...
        >
          ...
          <div className="font-heading text-2xl text-[#e8eaf6]/80 mt-2">WINS KARNATAKA CONSTITUENCY</div>
          <div
        ...
        <div className="px-4 md:px-8">
          <Link to="/analytics/dashboard" className="inline-block mt-6 text-[#7986cb] hover:text-white text-sm no-print">← Back to Dashboard</Link>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {[
              ...
              <div key={s.l} className="bg-[#111530] border border-[#2a2f52] rounded-xl p-5">
                <div className="text-xs uppercase tracking-wide text-[#7986cb] font-heading">{s.l}</div>
                <div className="font-heading text-3xl font-bold text-[#e8eaf6] mt-1">{s.v}</div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-[#111530] border border-[#2a2f52] rounded-xl overflow-hidden mt-6">
            <div className="px-6 py-4 border-b border-[#2a2f52] font-heading font-bold text-[#e8eaf6]">Final Vote Count — All Parties</div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {sorted.map((p, i) => {
                    ...
                    const rankColor = i === 0 ? '#ffd700' : i === 1 ? '#c0c0c0' : i === 2 ? '#cd7f32' : '#7986cb';
                    const isWinner = i === 0;
                    const margin = isWinner ? '—' : `-${winner.votes - p.votes} votes`;
                    return (
                      <tr key={p.id}
                        className={`${isWinner ? '' : i % 2 === 0 ? 'bg-[#111530]' : 'bg-[#0d1240]'} hover:bg-[#1a1f3e]`}
                      ...
                      >
                        ...
                        <td className="px-6 py-3 text-[#e8eaf6] font-semibold">{p.name}</td>
                        <td className="px-6 py-3 text-2xl">{p.emoji}</td>
                        <td className="px-6 py-3 text-[#e8eaf6]">{p.votes.toLocaleString()}</td>
                        <td className="px-6 py-3">
                          <div className="text-[#e8eaf6] text-sm">{pct}%</div>
                          <div className="w-full h-1.5 bg-[#2a2f52] rounded-full mt-1 overflow-hidden">
                            <div className="h-full" style={{ backgroundColor: p.color, width: `${pct}%` }} />
                          </div>
                        </td>
                        <td className="px-6 py-3">
                          {isWinner
                            ? <span className="inline-block bg-[#ffd700] text-[#0a0e27] rounded-full px-3 py-0.5 text-xs font-heading font-bold">🏆 WINNER</span>
                            : <span className="text-[#7986cb] text-xs">—</span>}
                        </td>
          ...
          {/* Final bar chart */}
          <div className="bg-[#111530] border border-[#2a2f52] rounded-xl p-6 mt-6">
            <div className="font-heading font-bold text-[#e8eaf6] mb-3">Final Results — Vote Distribution</div>
            <div style={{ height: 280 }}><Bar data={barData} options={barOptions} /></div>
          </div>

          {/* Constituency winners */}
          <div className="bg-[#111530] border border-[#2a2f52] rounded-xl overflow-hidden mt-6">
            <div className="px-6 py-4 border-b border-[#2a2f52] font-heading font-bold text-[#e8eaf6]">Constituency-wise Winners</div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#0a0e27]">
                  <tr>
                ...
                <tbody>
                  {constituencies.map((c, i) => {
                    ...
                    return (
                      <tr key={c.id} className={i % 2 === 0 ? 'bg-[#111530]' : 'bg-[#0d1240]'}>
                        ...
                        <td className="px-6 py-3 text-[#e8eaf6]">{w.votes}</td>
                        <td className="px-6 py-3 text-[#7986cb]">{r?.party ?? '—'}</td>
                        <td className="px-6 py-3 text-xs text-green-400">+{w.votes - (r?.votes ?? 0)}</td>
                        <td className="px-6 py-3 text-[#e8eaf6]">{c.turnout}%</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="flex gap-4 mt-6 no-print">
            <button onClick={() => window.print()} className="border border-[#2a2f52] text-[#e8eaf6] hover:bg-[#1a1f3e] rounded-xl px-6 py-3">🖨️ Export as PDF</button>
            <button onClick={generateCSV} className="border border-[#1565c0] text-[#1565c0] hover:bg-[#0d2240] rounded-xl px-6 py-3">📊 Download CSV</button>
          </div>
        </div>
      </div>
    </AnalyticsLayout>
